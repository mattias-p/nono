            ...
            for number in clue.0.iter() {
                if *number > freedom {
                    for x1 in x0 + freedom..x0 + number {
                        puzzle.grid.fill(x1, y, transposed);
                    }
                }
                x0 += number + 1;
            }
    ...
    fn run(&self, puzzle: &mut Puzzle, transposed: bool) {
        for (y, clue) in puzzle.clues.horz(transposed).iter().enumerate() {
            ...
            for number in clue.0.iter() {
                ...
                if x + 1 < puzzle.clues.width(transposed)
                    && puzzle.grid.is_filled(x + 1, y, transposed)
                {
                    // pulling fill
                    while x + 1 < puzzle.clues.width(transposed)
                        && puzzle.grid.is_filled(x + 1, y, transposed)
                    {
                        x += 1;
                    }
            ...
            let mut x1 = puzzle.clues.width(transposed) + 1;
            for number in clue.0.iter().rev() {
                ...
                if x > 0 && puzzle.grid.is_filled(x, y, transposed) {
                    // pulling fill
